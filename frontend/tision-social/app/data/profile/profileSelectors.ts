    ...
    (users) => {
        ...
        users.forEach(user => {
            user.profile.interests.forEach(interest => {
                ...
                interestToUserMap[interest.name].add(user); // store user object
            });
        });
...
export const userswithSameInterests = createSelector(
    [usersWithProfile, interestToUserIdMap], // rename to interestToUserMap if you updated above
    (users, interestToUserMap) => {
        const relatedUsers = new Map<number, PublicUser>();
        users.forEach(user => {
            user.profile.interests.forEach(interest => {
                const others = interestToUserMap[interest.name];
                if (others) {
...
export const userToUsersMap = createSelector(
    ...
    (users) => {
        ...
        users.forEach(user => {
            const userInstitutions = user.profile.education.map(edu => edu.institution);
            users.forEach(otherUser => {
                if (user.id !== otherUser.id) {
                    const hasOverlap = otherUser.profile.education.some(edu =>
                        userInstitutions.includes(edu.institution)
                    );
                    if (hasOverlap) {
